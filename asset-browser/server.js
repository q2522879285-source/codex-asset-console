...
import { classifyAsset } from "./asset-classification.js";
...
const localDataRoot = path.resolve(process.env.CODEX_ASSET_CONSOLE_HOME || path.join(process.env.LOCALAPPDATA || path.join(os.homedir(), "AppData", "Local"), "CodexAssetConsole"));
const promptLibraryRoot = path.resolve(process.env.PROMPT_LIBRARY_ROOT || path.join(localDataRoot, "prompt-library"));
const threeDRegistryPath = path.resolve(process.env.THREE_D_TASKS || path.join(__dirname, ".three-d-reconstruction-tasks.json"));
const actionTrashRoot = path.resolve(process.env.ASSET_ACTION_TRASH || path.join(localDataRoot, "action-trash"));
...
const governanceRoot = path.resolve(process.env.CODEX_WORKSPACE_GOVERNANCE || path.join(localDataRoot, "governance"));
const midjourneyRegistryPath = path.resolve(process.env.MIDJOURNEY_WORKSPACE_REGISTRY || path.join(localDataRoot, "midjourney.json"));
...
if (apiToken.length < 32) throw new Error(`Codex Asset Console API token is missing or invalid: ${apiTokenPath}`);
...
const workspaceGovernance = new WorkspaceGovernance({
  ...
  moduleFiles: [
    ...
    { id: "asset-classification", label: "解释性资产分类", component: "backend", path: path.join(__dirname, "asset-classification.js") },
...
function generatedRootFor(configuredPath) {
  return path.resolve(String(configuredPath || path.join(localDataRoot, "generated")));
}

function normalizeConsoleAutomation(raw = {}) {
  const automation = normalizeAutomation(raw);
  automation.inbox.capturePolicy = "ticketed-only";
  automation.cleanup.enabled = false;
  automation.cleanup.purgeEnabled = false;
...
async function loadConfig() {
  try {
    ...
    return {
      ...
      projects,
      storage: { generatedRoot: generatedRootFor(config.storage?.generatedRoot) },
      features: { midjourney: config.features?.midjourney === true },
      automation: normalizeConsoleAutomation(config.automation),
    ...
    return {
      enabled: true,
      projects: [],
      storage: { generatedRoot: generatedRootFor("") },
      features: { midjourney: false },
      automation: normalizeConsoleAutomation(),
...
async function managedOutputs(projectId = "") {
  ...
  const [generated, midjourney] = await Promise.all([
    ...
    config.features.midjourney ? midjourneyWorkspace.listOutputs({ projectId }) : []
  ]);
...
function managedClassification(output) {
  const smartGroup = output.review?.userStatus === "pending" ? "review" : "official";
  ...
  return {
    ...
    classification: {
      ...
      source: "registry",
      confidence: 1,
      reason: output.generator === "midjourney" ? "Midjourney 精确导入登记" : "生成任务输出登记"
    }
  };
}
...
async function moveAsset(body) {
  ...
  const canBindTarget = Boolean(movedGeneration?.threadId);
  if (canBindTarget) {
    try {
      taskBinding = await generationPipeline.bindThread({
...
async function listCases(projectId) {
  ...
  for (const scanRoot of project.scanRoots) {
    ...
    await appendCaseDirectories({ project, scanRoot, rootDir, maxDepth: 3, cases });
  }
  for (const output of await managedOutputs(project.id)) {
    if (!await exists(output.storePath)) continue;
    const stats = await fs.stat(output.storePath);
    const caseId = output.caseId || path.dirname(output.relativePath) || ".";
    const existing = cases.find((item) => path.normalize(item.id).toLowerCase() === path.normalize(caseId).toLowerCase());
    if (existing) {
      ...
      cases.push({
        id: caseId,
        projectId: project.id,
        name: path.basename(caseId === "." ? project.path : caseId).replace(/^\d{4}-\d{2}-\d{2}-/, ""),
        relPath: caseId,
        scanRoot: "@managed",
        mediaCount: 1,
        mtimeMs: stats.mtimeMs,
...
async function listAssets(projectId, caseId) {
  ...
  const files = caseExists ? await walk(caseDir) : [];
  ...
  for (const filePath of files) {
    ...
    assets.push({ ...asset, classification: classifyAsset(asset) });
  }
  for (const output of await managedOutputs(project.id)) {
    const outputCaseId = output.caseId || path.dirname(output.relativePath) || ".";
    if (path.normalize(outputCaseId).toLowerCase() !== path.normalize(caseId).toLowerCase()) continue;
    let stats;
    try {
...
function midjourneyTargetDirectory(project, requestedPath = "") {
  ...
  return requested ? path.join(requested, "midjourney") : path.join("imports", date, "midjourney");
}
...
async function saveAutomationSettings(body) {
  ...
  const automation = normalizeConsoleAutomation(body.automation || body);
...
const server = createServer(async (req, res) => {
  try {
    ...
    if (url.pathname === "/api/midjourney" && req.method === "GET") {
      if (!(await loadConfig()).features.midjourney) throw Object.assign(new Error("Midjourney adapter is disabled"), { statusCode: 404 });
    ...
    if (url.pathname === "/api/midjourney/import" && req.method === "POST") {
      const config = await loadConfig();
      if (!config.features.midjourney) throw Object.assign(new Error("Midjourney adapter is disabled"), { statusCode: 404 });
      const body = await readRequestBody(req);
      const projectId = String(body.projectId || "").trim();
      if (!projectId) throw new Error("请选择归属项目");
      const project = await getProjectStrict(projectId);
      ...
      const imported = await midjourneyWorkspace.importFiles({
        ...
        projectId: project.id,
        projectName: project.name,
        generatedRoot: config.storage.generatedRoot,
        targetRelativePath
      });
    ...
    sendJson(res, { error: error.message || String(error) }, Number(error.statusCode) || 500);
  }
});
