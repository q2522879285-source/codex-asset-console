...
const keyboardFavorite = await evaluate(`(async () => {
  ...
  favorite?.click();
  await wait(40);
  const stored = JSON.parse(localStorage.getItem('codex-asset-console:skill-favorites-v1') || '[]');
...
const interaction = await evaluate(`(async () => {
  ...
  favorite?.click();
  await wait(30);
  const storedAfterRemove = JSON.parse(localStorage.getItem('codex-asset-console:skill-favorites-v1') || '[]');
  ...
  const storedAfterRestore = JSON.parse(localStorage.getItem('codex-asset-console:skill-favorites-v1') || '[]');
