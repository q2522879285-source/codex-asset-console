  ...
  const result = buildHomeProjectShelf?.({
    ...
    tasks: [
      ...
      task({
        ...
        title: "最新执行任务",
      threadId: "01800000-0000-7000-8000-000000000003",
        updatedAt: "2026-08-10T03:00:00.000Z",
      }),
...
test("thread routes reject invalid ids and normalize local prefixes", async () => {
  ...
  assert.equal(
    threadRoute?.("01800000-0000-7000-8000-000000000001"),
    "/local/01800000-0000-7000-8000-000000000001",
  );
  assert.equal(
    threadRoute?.("local:01800000-0000-7000-8000-000000000001"),
    "/local/01800000-0000-7000-8000-000000000001",
  );
