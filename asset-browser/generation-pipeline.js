...
function defaultDestination(config, input, project, profile, date) {
  ...
  const kindLabel = input.kind === "video" ? "videos" : input.kind === "audio" ? "audio" : "images";
  const tail = path.join("outputs", date, kindLabel);
...
- 类型：${ticket.kind === "video" ? "视频" : ticket.kind === "audio" ? "音频" : "图片"}
- 生成工具：${ticket.generator || "未登记"}
- 模型：${ticket.model || "未登记"}
- 批次 / 章节：${ticket.episode || "-"}
- 场景：${ticket.scene || "-"}
- 镜头：${ticket.shot || "-"}
- 资产角色：${ticket.role || "-"}
- 版本：${ticket.version || "-"}
...
export class GenerationPipeline {
  ...
  async resolveRouting(input, config) {
    ...
    throw new Error("无法精确确定生成结果归属；请提供 projectId、profileId 或已绑定的 threadId");
  }
  ...
  async create(input, config) {
    return await this.withLock(async () => {
      ...
      const ticket = {
        ...
        profileName: profile?.name || "",
        kind,
        generator: cleanSegment(input.generator, `${kind}-generator`),
  ...
  async arm(ticketId, { sourcePath = "", expectedName = "" } = {}) {
    return await this.withLock(async () => {
      ...
      if (conflictingTicket) {
        throw new Error(`已有同类型下载任务正在等待：${conflictingTicket.id}。请先完成/取消它，或提供精确资产 ID 直接绑定。`);
      }
