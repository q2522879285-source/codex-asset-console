param(
  [int]$Port = 9231,
  [string]$InstallDir = (Join-Path $env:LOCALAPPDATA "Programs\Codex Asset Console"),
...
$stagingDir = Join-Path $installParent ".CodexAssetConsole-new-$PID"
$backupDir = Join-Path $installParent ".CodexAssetConsole-previous-$PID"
$backupMade = $false
$hadExisting = $false
...
$stateMarkerName = ".codex-asset-console-owned"
$stateMarkerValue = "Codex Asset Console state v1"
...
try {
  ...
  $expectedInstall = [IO.Path]::GetFullPath((Join-Path $env:LOCALAPPDATA "Programs\Codex Asset Console")).TrimEnd('\')
  $expectedState = [IO.Path]::GetFullPath((Join-Path $env:LOCALAPPDATA "CodexAssetConsole")).TrimEnd('\')
  if (-not $resolvedInstall.Equals($expectedInstall, [StringComparison]::OrdinalIgnoreCase) -or
    ...
    throw "InstallDir and StateDir must use the dedicated Codex Asset Console directories under LOCALAPPDATA"
  }
  ...
  if ($hadExisting) {
    ...
    if (-not (Test-Path -LiteralPath $existingManifestPath -PathType Leaf)) {
      throw "Existing destination is not owned by Codex Asset Console"
    }
    $existingManifest = Get-Content -LiteralPath $existingManifestPath -Raw | ConvertFrom-Json
    if ($existingManifest.name -ne "Codex Asset Console" -or
  ...
  if ($hadExistingState) {
    ...
    if (-not $markerValid -and -not $existingManifest -and $stateItems.Count -gt 0) {
      throw "Existing state directory is not owned by Codex Asset Console"
    }
  }
  ...
  if (-not $SkipShortcuts) {
    ...
    $desktopShortcut = Join-Path ([Environment]::GetFolderPath("Desktop")) "Codex 资产控制台.lnk"
    $programShortcut = Join-Path ([Environment]::GetFolderPath("Programs")) "Codex 资产控制台.lnk"
    $uninstallShortcut = Join-Path ([Environment]::GetFolderPath("Programs")) "卸载 Codex 资产控制台.lnk"
    $startupShortcut = Join-Path ([Environment]::GetFolderPath("Startup")) "Codex 资产控制台后台.lnk"
