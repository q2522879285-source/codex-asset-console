param(
  ...
  [string]$StateDir = (Join-Path $env:LOCALAPPDATA "CodexAssetConsole"),
  [int]$DebugPort = 9231,
  [switch]$SkipStart,
...
$backendPayload = Join-Path $runtimeRoot "service"
$frontendZip = Join-Path $runtimeRoot "codex-asset-console-windows.zip"
$manifestPath = Join-Path $runtimeRoot "manifest.sha256.json"
$ownedBackendRoot = Join-Path $env:LOCALAPPDATA "CodexAssetConsoleService"
$backupRoot = Join-Path $env:LOCALAPPDATA "CodexAssetConsole\backups"
$tempRoot = Join-Path ([IO.Path]::GetTempPath()) ("CodexAssetConsole-{0}-{1}" -f $PID, [Guid]::NewGuid().ToString('N'))
...
function Assert-BundleIntegrity {
  ...
  if (-not (Test-Path -LiteralPath $frontendZip)) { throw "Missing console package." }
  $manifest = Get-Content -LiteralPath $manifestPath -Raw -Encoding UTF8 | ConvertFrom-Json
  $zipHash = (Get-FileHash -LiteralPath $frontendZip -Algorithm SHA256).Hash.ToLowerInvariant()
  if ($zipHash -ne $manifest.consoleZipSha256) { throw "Console package hash mismatch." }
  foreach ($entry in @($manifest.files | Where-Object { $_.path.StartsWith('service/') })) {
    $relative = $entry.path.Substring('service/'.Length) -replace '/', '\'
...
function Ensure-ApiToken {
  ...
  if (Test-Path -LiteralPath $tokenPath) {
    $token = (Get-Content -LiteralPath $tokenPath -Raw -Encoding UTF8).Trim()
    if ($token.Length -lt 32) { throw "Existing local-service API token is invalid: $tokenPath" }
    return
  }
...
function Stop-ExactBackendProcess {
  ...
  if ($script:backendWasHealthy -and $processes.Count -eq 0) {
    throw "The local service is active on port 5177, but its process identity cannot be verified. Stop that service and retry."
  }
...
function Start-BundledBackend {
  $serverPath = Join-Path $BackendDir 'server.js'
  if (-not (Test-Path -LiteralPath $serverPath)) { throw "Local service is missing: $serverPath" }
  $tokenPath = Join-Path $BackendDir '.api-token'
  if (-not (Test-Path -LiteralPath $tokenPath)) { throw "Local-service API token is missing: $tokenPath" }
...
function Stop-BackendStartedByThisRun {
  ...
  if ($record -and $record.CommandLine -and $record.CommandLine.IndexOf($serverPath, [StringComparison]::OrdinalIgnoreCase) -ge 0) {
    ...
    if (Get-Process -Id $pidToStop -ErrorAction SilentlyContinue) {
      throw "Could not stop the local-service process started by this install; rollback was not attempted while it is active."
    }
  } elseif ($record) {
...
function Install-Backend($Manifest) {
  ...
  $script:backupDir = Join-Path $backupRoot ("service-{0}-{1}" -f (Get-Date -Format 'yyyyMMdd-HHmmss'), $PID)
  New-Item -ItemType Directory -Force -Path $script:backupDir | Out-Null
  $script:backendChanged = $true

  $entries = @($Manifest.files | Where-Object { $_.path.StartsWith('service/') -and -not $_.path.EndsWith('asset-browser.config.example.json') })
...
try {
  ...
  if (-not $FrontendOnly) {
    ...
    throw "Frontend-only install requires an existing authenticated local service."
  }
  ...
  if (-not $SkipStart) {
    ...
    if (-not $healthy) { throw "The local service did not become healthy on 127.0.0.1:5177." }
  }

  if (-not $BackendOnly) {
    $frontendTemp = Join-Path $tempRoot 'console'
    ...
    if (-not (Test-Path -LiteralPath $installer)) { throw "Console installer is missing from the package." }
    $installerParams = @{
      Port = $DebugPort
    ...
    if ($LASTEXITCODE -and $LASTEXITCODE -ne 0) { throw "Console installer exited with code $LASTEXITCODE" }
  }

  [pscustomobject]@{ installed = $true; consoleDir = $ConsoleDir; backendDir = $BackendDir; backupDir = $backupDir } | ConvertTo-Json -Depth 4
} catch {
  ...
  if ($backendWasHealthy -and -not (Test-BackendHealthy) -and (Test-Path -LiteralPath (Join-Path $BackendDir 'server.js'))) {
    ...
    if (-not (Test-BackendHealthy)) { Write-Warning 'Previous local-service files were restored, but the service did not restart.' }
  }
  throw
} finally {
