...
async function fixture() {
  ...
  const generatedRoot = path.join(root, "ManagedStore");
  await Promise.all([mkdir(inbox), mkdir(project), mkdir(generatedRoot)]);
  const config = {
    projects: [{ id: "project-a", name: "Project A", path: project, scanRoots: ["."] }],
    storage: { generatedRoot },
    automation: normalizeAutomation({
      inbox: { enabled: true, sourcePath: inbox, transferMode: "move" },
      routing: {
        enabled: true,
        profiles: [{ id: "project-a", name: "Project A", projectId: "project-a", keywords: ["project a"] }]
      }
    })
  };
  ...
  return { root, inbox, project, generatedRoot, config, pipeline };
}
...
test("unmatched generated video fails closed without exact ownership", async (t) => {
  const { root, config, pipeline } = await fixture();
  t.after(() => rm(root, { recursive: true, force: true }));

  await assert.rejects(pipeline.create({
    kind: "video",
...
test("armed watcher claims only the exact generated filename", async (t) => {
  ...
  t.after(() => rm(root, { recursive: true, force: true }));
  const ticket = await pipeline.create({ kind: "video", projectId: "project-a", generator: "video-generator" }, config);
  await pipeline.arm(ticket.id, { sourcePath: inbox, expectedName: "exact-result.mp4" });
  const unrelated = path.join(inbox, "family-video.mp4");
  const generated = path.join(inbox, "exact-result.mp4");
...
test("audio output is archived into the managed store as the only media copy", async (t) => {
  const { root, inbox, generatedRoot, config, pipeline } = await fixture();
  t.after(() => rm(root, { recursive: true, force: true }));
  const source = path.join(inbox, "exact-audio.wav");
  await writeFile(source, "audio", "utf8");
  const ticket = await pipeline.create({ kind: "audio", projectId: "project-a", generator: "audio-generator" }, config);

  const attached = await pipeline.attach(ticket.id, { sourcePath: source }, config);

  await assert.rejects(stat(source), { code: "ENOENT" });
  assert.equal(path.relative(generatedRoot, attached.output.storePath).startsWith(".."), false);
  assert.equal(await readFile(attached.output.storePath, "utf8"), "audio");
  assert.equal(attached.output.relativePath.endsWith(".wav"), true);
});
...
test("structured task text does not substitute for exact ownership", async (t) => {
  const { root, config, pipeline } = await fixture();
  t.after(() => rm(root, { recursive: true, force: true }));
