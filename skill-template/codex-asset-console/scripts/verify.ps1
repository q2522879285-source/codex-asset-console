...
Add-Check 'bundle:console-zip' ($zipHash -eq $manifest.consoleZipSha256) $zipHash
foreach ($entry in @($manifest.files | Where-Object { $_.path.StartsWith('service/') })) {
  ...
  $payload = Join-Path (Join-Path $runtimeRoot 'service') $relative
  $actual = if (Test-Path -LiteralPath $payload) { (Get-FileHash -Algorithm SHA256 -LiteralPath $payload).Hash.ToLowerInvariant() } else { '' }
  Add-Check "bundle:service/$relative" ($actual -eq $entry.sha256) $actual
}
...
foreach ($relative in $frontendFiles) {
  Compare-BundledFile "console/$relative" (Join-Path $ConsoleDir ($relative -replace '/', '\'))
}
...
foreach ($relative in $backendFiles) {
  Compare-BundledFile "service/$relative" (Join-Path $BackendDir ($relative -replace '/', '\'))
}
...
if (-not $SkipHealth) {
  try {
    ...
    Add-Check 'service:health' $true ("projects=" + @($response.projects).Count)
  } catch {
