...
if (-not $resolvedInstall.Equals($expectedInstall, [StringComparison]::OrdinalIgnoreCase) -or
  ...
  throw "Uninstall refused: directories are not the dedicated Codex Asset Console paths"
}
...
if ($manifest.name -ne "Codex Asset Console") {
...
if (-not $SkipShortcuts) {
  $shortcutPaths = @(
    (Join-Path ([Environment]::GetFolderPath("Desktop")) "Codex 资产控制台.lnk"),
    (Join-Path ([Environment]::GetFolderPath("Programs")) "Codex 资产控制台.lnk"),
    (Join-Path ([Environment]::GetFolderPath("Programs")) "卸载 Codex 资产控制台.lnk"),
    (Join-Path ([Environment]::GetFolderPath("Startup")) "Codex 资产控制台后台.lnk")
  )
