...
function renderCodexTaskContext() {
  ...
  const cannotBind = !state.codexThreadId;
...
async function saveCodexProjectBinding(projectId) {
  if (!state.codexThreadId || !projectId) return;
  const data = await api("/api/generation/bindings", {
    method: "POST",
    body: JSON.stringify({
...
async function createCodexProject(event) {
  ...
  if (!isAbsoluteWindowsProjectPath(projectPath)) {
    els.codexNewProjectMessage.textContent = "请填写完整的 Windows 绝对路径。";
...
async function loadConfig() {
  ...
  state.features = data.features || { midjourney: false };
  document.querySelectorAll("[data-open-midjourney], #midjourneyButton").forEach((element) => { element.hidden = !state.features.midjourney; });
...
async function loadProjects() {
  ...
  if (!state.selectedProject) {
    state.selectedProject = state.projects.find((project) => project.exists)?.id || "";
  }
...
function commitCases(entry, projectId) {
  ...
  if (!state.cases.some((item) => item.id === state.selectedCase)) {
    const defaultCase = state.cases[0];
...
function renderComparePanel() {
  ...
  for (const asset of assets) {
    ...
    curate.type = "button";
    curate.textContent = "移动到项目…";
    curate.addEventListener("click", () => openMoveAssetDialog(asset).catch(showError));
...
function shouldUseSmartBatches(assets) {
  return state.sort === "newest"
    && assets.length >= 4
    && new Set(assets.map((asset) => asset.directory || "")).size >= 2;
}
  ...
  card.innerHTML = `
    <div class="preview">
      ...
      <div class="badge-row">
        ...
        <span class="badge smart-group-badge group-${classification.group}">${classification.label}</span>
      </div>
...
function renderDetail() {
  ...
  els.detailBody.innerHTML = `
    ...
    <div class="detail-actions">
      ${state.codexEmbedded ? '<button id="useInCodex" class="codex-primary-action">附加到当前对话</button><button id="curateAsset">移动到项目…</button>' : ""}
  ...
  els.detailBody.querySelector("#curateAsset")?.addEventListener("click", () => openMoveAssetDialog(asset));
...
function preferredMoveTargetProjectId(assets, explicitProjectId = "") {
  ...
  return available.find((project) => !sourceProjects.has(project.id))?.id
    || assets[0]?.projectId
    || available[0]?.id
    || "";
}
...
function directProjectAutoOrganizePlan(project, currentCase, assets, availableDirectories = [], taskTitle = "", now = new Date()) {
  ...
  const organizeInsideCurrentCase = true;
...
async function openMoveAssetDialog(asset = state.selectedAsset, options = {}) {
  ...
  showMoveAssetMessage("点“自动整理”按项目结构和素材类型归位，也可以手动选择位置。");
...
async function routeSelectedAssetsForCurrentTask() {
  const assets = selectedAssets();
  if (!assets.length) return;
  const viewingBoundProject = state.codexBoundProject && state.selectedProject === state.codexBoundProject;
  if (!state.codexBoundProject || viewingBoundProject) {
    if (els.batchBarMessage) els.batchBarMessage.textContent = "请选择目标项目和目录。";
    await openBatchMoveDialog();
    return;
  }
  await routeAssetsToProject(assets, state.codexBoundProject, `移动到 ${projectLabel(state.codexBoundProject)}`);
}
...
async function routeAssetForCurrentTask(asset) {
  ...
  await routeAssetsToProject([asset], state.codexBoundProject, `移动到 ${projectLabel(state.codexBoundProject)}`);
}
...
function openBatchDeleteDialog() {
  ...
  if (assets.some((asset) => asset.managed)) {
    showToast("受管生成资产只在配置的资产仓保存一份；请先标记为“丢弃”，暂不物理删除", { tone: "error", duration: 6000 });
    return;
  }
...
async function openMidjourneyWorkspace() {
  ...
  const preferred = [state.codexBoundProject, state.selectedProject].find((id) => state.projects.some((item) => item.id === id && item.exists));
...
async function importSelectedMidjourney() {
  ...
  try {
    ...
    await loadMidjourney();
    showToast(`已将 ${data.imported.length} 张 MJ 图片移动到「${codexProjectName(projectId)} / ${data.targetRelativePath}」`);
  } finally {
...
function wireFilters() {
  ...
  els.batchCurateAssets?.addEventListener("click", () => openBatchMoveDialog().catch(showError));
  ...
  els.generationKind.addEventListener("change", () => {
    els.generationGenerator.value = `${els.generationKind.value}-generator`;
  });
